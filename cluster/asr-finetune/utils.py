#
""" Collection of utility functions for pre-processing, saving
...
def load_and_prepare_data_for_eval(path,feature_extractor,tokenizer,test_size=0.2, seed = 0):
    ...
    def prepare_dataset(batch):
        ...
        audio = batch["audio"]
        # compute log-Mel input features from input audio array
        batch["input_features"] = feature_extractor(audio["array"], return_attention_mask=True, sampling_rate=audio["sampling_rate"]).input_features[0]
...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features): # : List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        #     for feat in feature: print(feat.shape) return_attention_mask=True,
        # pdb.set_trace()
